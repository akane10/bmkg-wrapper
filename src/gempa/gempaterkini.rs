...
fn parse_data(xml: &str) -> Result<Vec<Gempa>, Box<dyn Error>> {
...
pub async fn get_data() -> Result<Vec<Gempa>, Box<dyn Error>> {
    let xml = fetch_data(Url::GempaTerkini.to_str()).await?;
    parse_data(&xml)
}
...
mod tests {
    ...
    // assert_eq!(parse_data(data).unwrap().tanggal, None);
    // }
}
